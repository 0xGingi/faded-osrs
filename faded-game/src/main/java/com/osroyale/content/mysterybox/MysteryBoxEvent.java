...
public class MysteryBoxEvent extends TickableTask {
    ...
    private void reward() {
        ...
        if (reward.rarity == MysteryRarity.EXOTIC) {
            World.sendMessage("<icon=17><col=5739B3> Faded: <col=" + player.right.getColor() + ">" + player.getName() + " </col>has won " + Utility.getAOrAn(reward.getName()) + " <col=5739B3>" + reward.getName() + " </col>from the <col=5739B3>" + mysteryBox.box.name() + "</col>.");
        }
